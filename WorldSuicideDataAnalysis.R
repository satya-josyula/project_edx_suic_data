...
# Combine some economic indicators of the countries in the world with the suicide rates data available and find any correlations between these indicators and suicide rates.
...
new$geometry <- NULL # This column can not be used for analysis
...
# We achieved an RMSE of about 11 before. We used simple Cross Validation at 10 during the RFE. Now we use Repeated Cross Validation with 2 repeats . As the training is slow as a results of the number of variables, we limit the number of trees to 100.
...
# There is a dramatic improvement in the RMSE with mtry of 12 with 100 trees. We achieved the best RMSE of about 3.
...
testnew %>% mutate(diff = predictions_rfrmsetrainplus_tune-suic_rate) %>% group_by(countryid) %>% summarize(rmse=sqrt(sum(diff*diff))) %>% knitr::kable(col.names = c("Method")) %>% kable_styling(bootstrap_options = c("striped", "condensed"))
testnew %>% mutate(diff = predictions_rfrmsetrainplus_tune-suic_rate) %>% group_by(sexid) %>% summarize(rmse=sqrt(sum(diff*diff))) %>% knitr::kable(col.names = c("Method")) %>% kable_styling(bootstrap_options = c("striped", "condensed"))
testnew %>% mutate(diff = predictions_rfrmsetrainplus_tune-suic_rate) %>% group_by(agegroupid) %>% summarize(rmse=sqrt(sum(diff*diff))) %>% knitr::kable(col.names = c("Method")) %>% kable_styling(bootstrap_options = c("striped", "condensed"))
testnew %>% mutate(diff = predictions_rfrmsetrainplus_tune-suic_rate) %>% group_by(generationid) %>% summarize(rmse=sqrt(sum(diff*diff))) %>% knitr::kable(col.names = c("Method")) %>% kable_styling(bootstrap_options = c("striped", "condensed"))
...
rm(fit.rfrmsetrainplus_tune,resultslmallmore,res2,res1,resultsnballmoref,resultsldaallmoref,resultstreebagallmore,resultstreebagallmoref,corMatrixall,highCorallmore,resultsnballf,resultstreebagall)
rm(resultstreebagallf,resultsrfall,resultsrfallf,corMatrix,highCorall,predictions_rfcattrainplus_tune)
